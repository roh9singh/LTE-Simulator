classdef bestCqiSchedulerMIMO < network_elements.lteScheduler
...
% www.nt.tuwien.ac.at
   ...
   methods
       function obj = bestCqiSchedulerMIMO(RB_grid_size,Ns_RB,UEs_to_be_scheduled,scheduler_params,CQI_params,averager,mapping_data,varargin)
       ...
       function UE_scheduling = scheduler_users(obj,subframe_corr,total_no_refsym,SyncUsedElements,UE_output,UE_specific_data,cell_genie,PBCHsyms)
           ...
           for u_=1:N_UE % calculate sum efficiencies for both codewords on every RB
               ...
               eff_temp(:,u_) = sum(reshape([obj.CQI_params(UE_output(u_).CQI(:,:,1:UE_scheduling(u_).nLayers)).efficiency],obj.RB_grid_size*2,UE_scheduling(u_).nLayers),2);
