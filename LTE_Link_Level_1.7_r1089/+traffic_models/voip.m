...
methods
    ...
    function check_TTI(obj)  
        if ~mod(obj.UE.clock.current_TTI-1,20)    % here comes the two state markov model assumed for the speech process (state changes every 20 ms)
            coin_toss = rand;
            ...
            if obj.state % generate a new speech packet in the active state 
                ...
                if ~mod(obj.UE.clock.current_TTI-1,160) % generate a new silence descriptor (SID) packet if the deadline is zero
