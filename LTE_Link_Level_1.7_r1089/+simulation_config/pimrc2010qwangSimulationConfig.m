classdef pimrc2010qwangSimulationConfig < simulation_config.simulatorConfig
    methods (Static)
        function LTE_params = apply_parameters(LTE_params,CFO,freq_sync_method,channel_type,mode,nRX,nTX)
            ...
            LTE_params.Bandwidth = 5e6;            % in Hz, allowed values: 1.4 MHz, 3 MHz, 5 MHz, 10 MHz, 15 MHz, 20MHz => number of resource blocks 6, 15, 25, 50, 75, 100
            LTE_params.introduce_timing_offset =  false;
            LTE_params.introduce_frequency_offset =  logical(CFO);
            %% Define some User parameters (identical settings).
            LTE_params.UE_config.channel_estimation_method = 'PERFECT';      %'PERFECT','LS','MMSE'
            LTE_params.UE_config.mode = mode;                     % DEFINED IN STANDARD 3GPP TS 36.213-820 Section 7.1, page 12
            % 1: Single Antenna, 2: Transmit Diversity, 3: Open Loop Spatial Multiplexing
            % 4: Closed Loop SM, 5:
            % Multiuser MIMO
            LTE_params.UE_config.nRX = nRX;                      % number of receive antennas at UE
            ...
            LTE_params.UE_config.carrier_freq_offset = CFO;   % carrier frequency offset normalized to subcarrier spacing
            LTE_params.UE_config.freq_sync_method = freq_sync_method; % carrier frequency offset estimation 'perfect', 'estimated', 'none'
            LTE_params.UE_config.rfo_correct_method = 'subframe'; % 'none','subframe'
            %% Define BS parameters (identical settings).
            LTE_params.BS_config.nTx = nTX;
