classdef mobilkom_CLSM < simulation_config.simulatorConfig
    % Simulation parameters for a SUMIMO simulation
    ...
    methods (Static)
        function LTE_params = apply_parameters(LTE_params)
            ...
            LTE_params.UE_config.user_speed = 1000/3.6;    %[m/s]
            ...
            LTE_params.ChanMod_config.type = 'winner_II'; % 'PedA', 'PedB', 'PedBcorr', 'AWGN', 'flat Rayleigh','VehA','VehB','TU','RA','HT','winner_II'
            %% Scheduler settings
            ...
            %% mobilkom specific settings
            LTE_params.antennas.spacing = 1/2;
            LTE_params.antennas.TX_type = 'linear';
            LTE_params.antennas.RX_type = 'linear';
        end
    end
end
